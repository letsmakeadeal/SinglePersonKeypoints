...
import nn_constructors.loss_heads as LossHeads
import pipeline.metrics as CustomMetrics
import nn_constructors.backbones as Backbones
...
def build_backbone_from_cfg(config) -> Tuple[torch.nn.Module, int]:
    ...
    elif hasattr(segmentation_models_pytorch, backbone_type_name):
        backbone = getattr(segmentation_models_pytorch, backbone_type_name)(**args)
    else:
